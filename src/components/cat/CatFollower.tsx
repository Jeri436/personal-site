...
function frames(folder: string, name: string, count: number) {
  return Array.from({ length: count }, (_, i) => `/cat/${folder}/${name}_${i}.png`);
}
...
const FRAME_MS: Record<Motion, number> = {
  idle: 110, idle2: 110, walking: 80, running: 48,
};
...
export default function CatFollower() {
  ...
  function scheduleWander(delayMs?: number) {
    ...
    const delay = delayMs ?? (WANDER_PAUSE_MIN + Math.random() * (WANDER_PAUSE_MAX - WANDER_PAUSE_MIN));
  ...
  useEffect(() => {
    const isIdleAnim = motionRef.current === 'idle' || motionRef.current === 'idle2';
    let i = 0;
  ...
  useEffect(() => {
    ...
    if (wrapRef.current) {
      wrapRef.current.style.transform =
        `translate(${cx - SIZE / 2}px, ${cy - SIZE / 2}px)`;
    }
    scheduleWander(1500);
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      if (wanderTimerRef.current) { clearTimeout(wanderTimerRef.current); wanderTimerRef.current = null; }
  ...
  useEffect(() => {
    const tick = () => {
      ...
      if (dist > ARRIVE_THRESH) {
        if (arriveTimerRef.current) { clearTimeout(arriveTimerRef.current); arriveTimerRef.current = null; }

        const motion: Motion = isWanderingRef.current
          ? 'walking'
          : dist > RUN_THRESH ? 'running' : 'walking';
        ...
        const isMoving = motionRef.current === 'walking' || motionRef.current === 'running';
      ...
      if (wrapRef.current) {
        wrapRef.current.style.transform =
          `translate(${cat.x - SIZE / 2}px, ${cat.y - SIZE / 2}px)`;
      }
